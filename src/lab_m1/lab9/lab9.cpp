...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(2.5f, 0.5f, 0));
...

    {
        ...
        glm::vec3 object = glm::vec3(1.0f, 0.5f, 0.0f);
        glm::vec3 direction = glm::normalize(camera - object);
    ...
    // mix texture
    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(180.0f), glm::vec3(1, 0, 0));
        RenderSimpleMesh(meshes["square"], shaders["LabShader"], modelMatrix, mapTextures["crate"], mapTextures["grass"]);
        mix = 0;
    }
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 &modelMatrix, Texture2D *texture1, Texture2D *texture2)
{
    ...
    // Set mixer uniform
    int loc_mixer = glGetUniformLocation(shader->program, "mixer");
    glUniform1i(loc_mixer, mix);

    // bonus: Apply rotation to the earth (chaning the texture coordinates in the vertex shader)
    ...
    glUniform1i(glGetUniformLocation(shader->program, "mix"), mix);
