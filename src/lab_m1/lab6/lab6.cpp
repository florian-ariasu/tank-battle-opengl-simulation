...
void Lab6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
    ...
    // Set shader uniform "Time" using Engine's elapsed time
    if (timeLocation != -1) {
        glUniform1f(timeLocation, (GLfloat)Engine::GetElapsedTime());
    }

    int timeLoc = glGetUniformLocation(shader->GetProgramID(), "Time");
    glUniform1f(timeLoc, abs(sinf(time)) + abs(sinf(time)));
